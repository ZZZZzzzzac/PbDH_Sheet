...
System Package 还可以在 `manifest.json` 声明一个可选 **Sheet Shell**。Shell 是页面之外的持久布局，用于让某些模块在切页时保持挂载：
...
<main class="workspace">
  <section class="character-pages">
    <pb-page-outlet></pb-page-outlet>
  </section>
  <aside class="persistent-cards">
    <pb-module id="domain-card-table"></pb-module>
  </aside>
</main>
```

- `<pb-page-outlet>` 是 Current Page 的挂载位置；一个 Shell 必须且只能声明一个。
- Shell 中的 `<pb-module>` 是 Persistent Module，切换 Current Page 时不会卸载。
- 页面 HTML 只描述 outlet 内会切换的内容，不应再次声明 Shell 中已有的同一个模块。
- Shell CSS 与页面 CSS 分开：Shell CSS 控制固定栏位，页面 CSS 控制各自内容。
- 未声明 `shell` 的旧 System Package 保持兼容：导航和页面直接渲染在 Sheet Tool 根部。
- 导出预览时，Page Outlet 渲染全部可打印页面；Persistent Module 只渲染一次，不会随每页重复。
...
- Shell 页面出口：`<pb-page-outlet></pb-page-outlet>`，只允许出现在 Sheet Shell。
