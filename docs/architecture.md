...
静态托管应用
  ...
  |     临时线性步骤、目标滚动/高亮、Restricted Markdown 说明
  |
  +-- validation-runner
...
Sheet Module 事件
  ...
  -> 保存必要的 Derived Source Snapshot，并执行一次性 Character Data 更新
  -> state-store 调用 dependency-engine
  -> dependency-engine 从全部可用来源重新计算纯派生视图，并对当前事件计算一次性补丁
  -> store 提交允许的补丁
...
Author-facing 接口见：[System Package 文档中心](system-package/README.md)。
...
- MVP 单轮计算，不做链式触发。
- 载入、导入或切换 Character Save 时只重建 `setVisibility`、Resource Picker 默认筛选和 `readOnlyDisplay` 内容，不重放 Character Data patch 或 Card 创建。
...
- MVP 不支持链式触发。
- 为纯派生动作读取 Derived Source Snapshot 或现有 Composite Resource；失效引用只报告 warning。
- 纯派生重建从默认状态开始，不持久化最终 view/option effects，也不执行一次性 data patches。
...
- 只读取 Guide definition、临时 step index 和稳定 page/module/region 目标的显示几何。
- 不读取 Character Data，不执行 Author 脚本，不解释或请求游戏规则行为。
- 只输出遮罩、滚动、高亮、说明、目标不可见提示和手动导航 UI。
- 目标属于另一 Runtime-Visible Page 时只切换临时 Current Page；退出不恢复启动前页面。
- 不发出资源选择事件，不执行跨模块 Character Data patch，不操作 Cards，不触发 Validation Runner。
- 高亮 Sheet Module 保持普通交互，所有实际行为仍由模块及既有引擎负责。

每个 Guide Step 声明稳定 ID、纯文本标题、Restricted Markdown 说明，以及可选的单一页面、Sheet Module 或 Layout Region 目标，用于校验、预览和 AI 辅助生成。Layout Region 是带 package-wide unique `data-guide-region-id` 的安全静态 HTML 容器；Spotlight 将其作为单个矩形，保持容器子树中的 Sheet Modules 可交互。说明面板与 Guide actions 分离布局，长说明不会把导航操作挤出可达范围。
