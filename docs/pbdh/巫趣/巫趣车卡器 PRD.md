...
人物卡只有一个页面，不设置 Card Table 或持久 Shell。桌面端人物卡在工作区水平居中，保持 210:297 的 A4 比例；窄屏允许页面缩放或滚动，但所有输入区域必须可到达。打印时只生成一张 A4 人物卡。主体采用左右栏：左栏顶部的大立绘与姓名/原型/终末条件容器并排，原型规则在其下方横跨左栏，再衔接经历与道具；右栏顶部为三项魔法本质、魔力点与蚀痕，并直接衔接魔法区。
...
3. Player 可以分别编辑魔力点当前值与蚀痕数量，并让系统自动计算魔力点上限。
