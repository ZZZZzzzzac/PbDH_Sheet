...
框架标识字段 `ID` 始终保留在 normalized Entry 中供引用和验证使用，但推断 metadata 默认设置为不可见、不可筛选、不可排序、不可搜索，因此 Resource Picker 默认不向 Player 展示 ID。Author 如确有诊断或展示需要，可在对应 `资源库[]` 链接的 `字段模板` 中显式声明 `键: "ID"` 并启用相应选项。
