...
`fillText` 写入的文本与 `fillCountable` 写入的 `{current,max}` 都是持久化 Sheet Values。`fillText` 的模板只在 Dependency 触发时生成普通文本；追加模式把新文本合并进现有 freeText/longText，Player 后续可继续自由编辑，不保存模板或 Resource Entry 引用。`fillCountable` 未指定的成员保留原值；动态 max 属于 Player save，而不是对 System Package Module 配置的修改。
