...
- Source `{类型:"freeText", 模块ID}` ↔ trigger `{类型:"freeTextChanged", 来源模块ID}`。普通输入和下拉选择的 `onChange`继续保存 Character Data；只有离开控件时提交最终字符串并求值。`freeTextChanged` 第一版只允许 `setResourceDefaultFilter`。
...
- `setTextPlaceholder`：`{目标模块ID, 内容}`，目标只能是 freeText/longText，内容使用 `fillText` 的字符串、`selectedResourceField` 或 `selectedResourceTemplate`合同。只允许 `resourceSelected` 触发。结果是纯派生 placeholder，不写 Character Data；Text Module 的静态`占位文本`作为无派生结果时的回退。
- `setResourceDefaultFilter`：`{目标模块ID, 字段, 值}`，目标必须 resourcePicker，目标字段必须存在于目标 Library。`值`可为至少一项的 `string[]`，也可为 `{类型:"selectedResourceField", 字段, 选择索引?}`，在 `resourceSelected` 时从选中 Entry 动态取得筛选值；后者的来源字段必须存在，因而一条规则可适用于扩展新增的同类资源，而无需枚举 Entry 名称或 ID。
  - `freeTextChanged` 可使用 `{类型:"freeTextValues", 模块IDs:string[]}`。每个 Module ID 必须指向 Free Text，并以 `{类型:"freeText", 模块ID}`列入同一 Rule 的 `sources`。Engine 从当前 Character Data 读取这些值、去除首尾空白、忽略空值并去重，生成同一字段内的 OR 筛选。
...
Free Text 已是 Character Data 中的普通字符串，不需要 Derived Source Snapshot。加载、导入或切换 Character Save 时，Engine 直接从声明的 Free Text sources 重建 `freeTextValues` 默认筛选；不会伪造 blur、重放数据写入或让 Browser 读取 Character Data。
