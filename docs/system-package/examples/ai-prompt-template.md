...
4. countableResource: ID, 类型, 标签, 最小值?, 最大值?, 默认值?, 步长?, 最大值可改?, 显示方式?(数值|标记), 当前值标记?, 剩余值标记?, 标记尺寸?, 加减号字号?, 默认隐藏?；标记展示时两字段各为 `{类型:"文字",内容:"单个字素"}` 或 `{类型:"图片",资源路径:"assets/**"}`，可混用但必须不同，最小值不得为负
...
9. cardTable: ID, 类型, 标签, 资源来源, 状态选项?, 状态外观?(Record<state,{描边颜色:#RRGGBB,徽标:non-empty string}>), 显示方式?, 卡图字段?, 卡背字段?, 显示方式字段?, 背面卡牌ID字段?, 默认隐藏?
