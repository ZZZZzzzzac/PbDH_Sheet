...
本页给出一套自包含的 `schemaVersion: 0.2.0` 示例。它刻意展示所有可选字段和典型组合；真实包应删除不用的声明，而不是保留空功能。
...
```json
{
  ...
  "skins": [
    { "ID": "plain", "名称": "简洁", "css": "skins/plain.css", "推荐框架配色": "light" },
    {
      "ID": "editorial",
      "名称": "编排版",
      ...
      "推荐框架配色": "dark",
      "layoutOverrides": {
        "pages": [{ "ID": "main", "html": "skins/editorial/main.html" }]
      }
    }
  ],
  "defaultSkin": "plain",
...
可选字段是 `shell`、`skins`/`defaultSkin`、`dependencies`、`characterCreationGuide`、`resourceLibraries`、`validationChecks`。图片放入 `assets/**` 后自动发现。
...
[
  ...
  },
  {
    ...
    "最大值可改": true,
    "显示方式": "标记",
    "当前值标记": { "类型": "图片", "资源路径": "assets/logo.svg" },
    "剩余值标记": { "类型": "文字", "内容": "◇" },
    "标记尺寸": 20,
    "加减号字号": 18,
    "默认隐藏": false
...
| countableResource | `最小值`、`最大值`、`默认值`、`步长`、`最大值可改`、`显示方式`、`当前值标记`、`剩余值标记`、`标记尺寸`、`加减号字号`；尺寸为 5–96 CSS px，标记对象可分别声明单一 Unicode 字素或 `assets/**`图片且必须不同，最小值非负 |
...
`skins/plain.css` 只设置共享视觉令牌；`skins/editorial/skin.css` 用同一份 CSS 覆盖全包。`skins/editorial/main.html` 必须包含 Base `main` Page 完全相同的 Module ID集合，未覆盖的 `secret` Page 与 Shell 自动使用 Base HTML。完整写法见 [Skin starter](skin-starter.md)。
...
7. Validation Checks。
8. Skins。
9. 运行 Validator，修复所有 fatal/error。
10. Author Preview 冒烟测试 Character Save、Picker、Cards、Guide、Checks 与输出。
