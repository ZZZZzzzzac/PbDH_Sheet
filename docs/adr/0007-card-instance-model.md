...
- 卡牌可以标记不同状态（如 configured、vault），但状态是卡牌实例的属性，不是物理区域隔离。
- MVP 支持创建实例、删除实例、拖拽移动、排序、状态切换。
- 翻面、横置/竖置、指示物数字等扩展操作列入后续信号，不在 MVP 范围。
...
- 未来扩展翻面、横置、指示物、自定义容器或更复杂卡牌状态时不需要重写核心模型。

## 代价

- Character Data 中卡牌状态比单纯 `cardId` 列表复杂。
- UI 需要提供移动端可用的按钮/菜单操作，不能只依赖拖拽。
- 翻面、横置、指示物等操作不在 MVP 范围，需要后续扩展。
...
初版 ADR 设想了离散容器模型（configured/vault/library 三个物理区域，卡牌在区域间移动）。开发过程中确定改为自由桌面模型：玩家在一个卡牌桌面区域内自由摆放卡牌，通过坐标和层级确定位置，不强制离散容器区域。卡牌仍可标记状态（如 configured、vault），但状态是实例属性而非物理区域隔离。
