...
- Questionnaire HTML runs in a Base-owned new-tab Host inside a sandboxed iframe without same-origin or network access. A Stable Resource Entry missing from the current Effective Resource Catalog is shown as a Base-owned confirmation warning and skipped while available selections remain applicable; if none are available, confirmation is disabled. Other invalid, cancelled, or stale results leave Character Data unchanged.
...
- Native Character Data compatibility is scoped by System Package ID, not Package version. Import projects source data onto a new Character Save built from the Current System Package defaults, retains only data the current package can use unchanged, reports and discards every incompatible item, and records the current Package version.
- Native Character Data import never overwrites an existing Character Save: every import receives a new Character ID. Lossless projection completes directly; lossy projection requires Player confirmation through the shared character-conversion report.
